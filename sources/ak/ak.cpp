...
int main(int argc, char *argv[]) {
	...
	if (argc != 2) {
        fprintf(stderr, "Usage: %s YOUR_PATH_TO_ENVIRONMENT\n", argv[0]);
        exit(EXIT_FAILURE);
    }

	AK_MSG_DBG("TASK LIST LEN: %d\n", ak_thread_table_len);

	task_init((const char*)argv[1]);
	...
	for (uint32_t index = 0; index < ak_thread_table_len; index++) {
		...
		/* create task */
		pthread_create(&(task_list[index].pthread), &(task_list[index].pthread_attr), task_list[index].task, &(task_list[index].id));
#if (AK_PRIORITY_ENABLE == 1)
