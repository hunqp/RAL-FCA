...
static in_addr_t resolveHost(const char *host) {
	in_addr_t saddr;
...
static int buildPackages(void *p, size_t *len) {
	struct timeval tv;
...
int netTimeProtocolsUpdate(const char *server) {
	int sfd, mfd;
	size_t nbytes;
	double offset;
    fd_set readfds;
	...
	servaddr.sin_addr.s_addr = resolveHost(server);
	...
	nbytes = BUFSIZE;
	if (buildPackages(chars, &nbytes) != 0) {
		close(sfd);
		return -3;
	}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
static void *drv_ntpd_run(void *args) {
	int to, rc = -1;
    uint32_t lastTs = time(NULL);
    bool bFirstTimeCompleted = false;
    int refreshAfterSecs = 3600;
    ...
    while (boolean) {
        to = (!bFirstTimeCompleted) ? 1 : refreshAfterSecs;

        if (time(NULL) - lastTs > to) {
            lastTs = time(NULL);

            for (uint8_t id = 0; id < totalDomains; ++id) {
                char *server = domains[id];
                if (!server) {
                    continue;
                }
                rc = netTimeProtocolsUpdate(server);
                if (rc == 0) {
                    bFirstTimeCompleted = true;
                    printf("Network time updates success by %s\r\n", server);
                    break;
                }
                else {
                    printf("Network time updates failure by %s", server);
                }
            }
        }
        sleep(1);
...
void NTPD_Begin(void) {
	if (!pid) {
		boolean = true;
		pthread_create(&pid, NULL, drv_ntpd_run, NULL);
	}
}
